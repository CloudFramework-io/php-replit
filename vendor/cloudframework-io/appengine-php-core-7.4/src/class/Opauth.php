...
class Opauth {
    ...
    private function requireStrategy($strategy) {
        if (!class_exists($strategy.'Strategy')){
            $found = false;
            if(is_file( __DIR__ . "/Opauth/{$strategy}Strategy.php")) {
                require_once __DIR__ . "/Opauth/{$strategy}Strategy.php";
            } else {
                trigger_error('Strategy class file ('.__DIR__ ."/Opauth/{$strategy}Strategy.php".') is missing', E_USER_ERROR);
            }
        }
...
class Opauth2{
     ...
     * Loads user configuration and strategies.
     *
     * @param Core $core cloudframework-io Core
     * @param array $config User configuration
     * @param boolean $run Whether Opauth should auto run after initialization.
     */
    public function __construct(Core7 &$core,$config = array(), $run = true){
    ...
    public function run(){
        ...
        if (!empty($this->env['params']['strategy'])){
            ...
            elseif (array_key_exists($this->env['params']['strategy'], $this->strategyMap)){
                ...
                _printe('Unsupported or undefined Opauth strategy - '.$this->env['params']['strategy'], E_USER_ERROR,$this->env['params']['strategy'],$this->strategyMap);
                trigger_error('Unsupported or undefined Opauth strategy - '.$this->env['params']['strategy'], E_USER_ERROR);
            }
        }
        else{
